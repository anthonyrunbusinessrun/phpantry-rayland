      ...
      <section style={{ padding: '100px 5%', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'start' }}>
          <div>
            ...
            <h2 style={{ fontSize: 'clamp(28px,4vw,44px)', color: 'var(--text)', marginBottom: 16 }}>Born from <span className="text-red">Compassion</span></h2>
            <div className="accent-line" />
            <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.85, marginBottom: 24 }}>PHPantry began as a response to the daily struggles of Filipino families trying to put food on the table. What started as a small act of compassion quickly grew into a community movement.</p>
            <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.85, marginBottom: 24 }}>With churches opening their doors and volunteers offering their time, each pantry became more than a place for food — it became a space of hope, dignity, and connection.</p>
            <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.85 }}>More than just giving, PHPantry is about sharing. It brings together communities, churches, volunteers, and partners to lift each other up — proving that even the smallest act of kindness can make a lasting difference.</p>
            ...
            <div style={{ marginTop: 48, position: 'relative', paddingLeft: 32 }}>
              ...
              {[
                ...
                { title: 'A Movement Grows', text: 'Today, PHPantry Philippines reaches families across the nation.' },
              ].map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 20, marginBottom: 28, position: 'relative' }}>
                  <div style={{ position: 'absolute', left: -32 + 9 - 8, top: 4, width: 18, height: 18, borderRadius: '50%', background: i === 3 ? 'var(--red)' : 'var(--blue)', border: '2px solid var(--bg)', zIndex: 1, boxShadow: `0 0 10px rgba(${i === 3 ? '198,40,40' : '21,101,192'},0.4)` }} />
                  <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Church card */}
            <div className="card" style={{ padding: 36, borderLeft: '3px solid var(--blue)' }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.8, marginBottom: 12 }}>Churches play a vital role in PHPantry Philippines. They serve as trusted community anchors — offering space, volunteers, and guidance to help each pantry operate smoothly.</p>
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.8 }}>More importantly, they bring heart into the mission, turning faith into action and service into something deeply meaningful for the community.</p>
            </div>

            {/* Quote card */}
            <div style={{ borderRadius: 6, padding: 36, textAlign: 'center', background: 'linear-gradient(135deg, #071830, #1565C0)', border: '1px solid var(--border)' }}>
              <div style={{ fontFamily: 'var(--font-cond)', fontStyle: 'italic', fontSize: 20, color: 'white', lineHeight: 1.55, marginBottom: 16 }}>
            ...
            {/* Founder card */}
            <div className="card" style={{ padding: 28, borderLeft: '3px solid var(--red)' }}>
              ...
              <div style={{ fontSize: 13.5, color: 'var(--text-muted)', lineHeight: 1.7 }}>Founder of PHPantry Philippines — driven by compassion and the belief that every Filipino deserves food with dignity.</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '100px 5%', background: 'linear-gradient(112deg, #071830 0%, #0D2468 35%, #1565C0 65%, #C62828 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, position: 'relative', zIndex: 1 }}>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 16 }}>
              {['Take only what you need', 'Give what you can spare', 'No expired or unsafe food donations', 'Practice proper food handling and cleanliness', 'Respect volunteers and fellow community members', 'Help keep the pantry clean and accessible'].map((item, i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, fontSize: 15, color: 'rgba(255,255,255,0.8)', lineHeight: 1.6 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: 40, textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'var(--font-script)', fontSize: 36, color: 'white', lineHeight: 1.4, marginBottom: 20 }}>
            ...
            <Link href="/get-involved" className="btn-primary" style={{ marginTop: 28, background: 'white', color: '#0D1B3E' }}>Find a Pantry</Link>
          </div>
        </div>
      </section>
    </>
