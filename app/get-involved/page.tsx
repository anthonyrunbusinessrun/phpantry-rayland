...
export default function GetInvolved() {
  ...
  return (
    <>
      ...
      {/* 3 WAYS */}
      <section style={{ padding: '80px 5%', background: 'var(--bg-card2)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {[
              ...
              <div key={i} className="card card-blue-glow" style={{ padding: '32px 26px', textAlign: 'center' }}>
                <div style={{ width: 64, height: 64, background: `${card.color}1A`, border: `1px solid ${card.color}44`, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 28 }}>{card.emoji}</div>
                <div style={{ fontFamily: 'var(--font-cond)', fontSize: 22, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>{card.title}</div>
      ...
      <section style={{ padding: '100px 5%', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px,4vw,44px)', color: 'var(--text)', marginBottom: 12 }}>Join the <span className="text-blue">PHPantry</span> Family</h2>
          <div className="accent-line" />
          <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.8, marginBottom: 40, marginTop: 16 }}>Fill out the form below and our team will reach out to you about the next steps to get involved.</p>

          {status === 'success' ? (
            <div style={{ padding: '40px 36px', background: 'var(--bg-card)', border: '1px solid rgba(46,125,50,0.3)', borderRadius: 6, textAlign: 'center' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div>
                ...
                <textarea placeholder="Tell us a little about yourself or how you'd like to help..." rows={5} value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} />
              </div>
